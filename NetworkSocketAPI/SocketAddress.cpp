...
static void ipv4_from_address(uint8_t *bytes, const char *addr) {
	...
	for (; count < NSAPI_IPv4_BYTES; count++) {
		int scanned = sscanf(&addr[i], "%d", &tmp);

		if (tmp > 255 || tmp < 0 )
		{
			assert (0  && " Incorrect parse adres ");
		}
		bytes[count] = (uint8_t) tmp;
...
SocketAddress::SocketAddress(NetworkStack *iface, const char *host,
		uint16_t port) {
...
SocketAddress::SocketAddress(const void *bytes, nsapi_version_t version,
		uint16_t port) {
	memset(&_ip_address, 0, sizeof _ip_address);
